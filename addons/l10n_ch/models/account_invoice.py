...
class AccountMove(models.Model):
    ...
    def _compute_l10n_ch_isr_number(self):
        ...
        """
        for record in self:
            is_qrbill = record.invoice_partner_bank_id and\
                        record.invoice_partner_bank_id._is_qr_iban() or\
                        False
    ...
    def isr_print(self):
        ...
        if self.l10n_ch_isr_valid:
            ...
            if not self.invoice_partner_bank_id:
                errors.append(
                    _("- Invoice's 'Bank Account' is empty. You need to create"
                      " or select a valid ISR account")
                )
            elif not self.l10n_ch_isr_subscription:
                errors.append(
                    _("- No ISR Subscription number are set on you company bank"
                      " account. Please fill it.")
                )
